...
internal_TokenKind CTurboTokenAnalysis::GetTokenKind(CTurboToken *token)
{
	if (token->tag() == "." || token->tag() == ":" || token->tag() == ",")
		return internal_TokenKind::sep_punct;
	else if (token->tag() == "\"" || token->tag() == "''")
		return internal_TokenKind::sep_quote;
	else if (token->tag() == "CD")
		return internal_TokenKind::number;
	else if (token->HasDigit())
		return internal_TokenKind::alphanum;
	else
		return internal_TokenKind::alpha;
}

internal_TokenKind CTurboTokenAnalysis::GetSepTokenKind(const char *text, int len)
{
	int i;
	for (i=0; i < len; i++)
	{
		if (text[i] == '\n' || text[i] == '\r')
			return internal_TokenKind::sep_paragraph;
		if (text[i] == '\"')
			return internal_TokenKind::sep_quote;
		if (text[i] == '-')
			return internal_TokenKind::sep_dash;
#if 0
	...
	return internal_TokenKind::sep_space;
}
